...
public class MapStageGenerator {
    ...
    private static final LandscapePart[] DOWN_FLOOR_SPRITES = { LandscapePart.CROWN_CLIMB,
            LandscapePart.STANDARD_CLIMB, LandscapePart.BOOT_CLIMB, LandscapePart.ROUND_CLIMB };
    private static final LandscapePart[] UP_FLOOR_SPRITES = { LandscapePart.MIRROR_CROWN_CLIMB,
            LandscapePart.MIRROR_STANDARD_CLIMB, LandscapePart.MIRROR_BOOT_CLIMB,
            LandscapePart.MIRROR_ROUND_CLIMB };
    ...
    public MapStageImpl convertDataToMapStage(
        ...
        for (int x = 0; x < stageLength; x++) {
            ...
            if (rawDataCeilingAndFloor.getSecondElement().get(index).getSecondElement() == LandscapeBehaviour.UP) {
                heightCeilingAndFloor.setSecondElement(heightCeilingAndFloor.getSecondElement() - 1);
            } else if (rawDataCeilingAndFloor.getSecondElement().get(index).getSecondElement() == LandscapeBehaviour.DW) {
                heightCeilingAndFloor.setSecondElement(heightCeilingAndFloor.getSecondElement() + 1);
            }
        }
        return returnStage;
